...
from utility.genutil import add_plural, dump_pickle, load_pickle
...
class JobLocationsAnalyzer(Analyzer):
    def __init__(self, conn, db_session, main_config, logging_config):
        ...
        self.locations_geo_coords = load_pickle(os.path.expanduser(
            self.main_config['data_filepaths']['cache_loc_geo_coords']))
    ...
    def _get_locations_geo_coords(self, locations, use_country_fallback=False):
        ...
        # Current locations' geographic coordinates
        cur_locs_geo_coords = {}
        counts = 0
        ...
        for i, (city, region, country) in enumerate(locations, start=1):
            ...
            if not location:
                ...
                continue
            elif location in cur_locs_geo_coords:
                self.logger.debug(
                    "Location '{}' is already added!".format(location))
                skipped_locs['already_added'].append(location)
                cur_locs_geo_coords[location]['count'] += 1
                counts += 1
                continue
            elif location in self.locations_geo_coords:
                # We already computed the location's latitude and longitude with
                # the geopy service
                geo_coords = self.locations_geo_coords.get(location)
                self.logger.debug(
                    "Location '{}' found in cache! Geo coordinates: {}".format(
                        location, geo_coords.point))
            else:
                try:
                    self.logger.debug("Sending request to geopy ...")
                ...
                # TODO: try to factorize since the same steps are repeated above
                # and below (try, except, else, if, ...)
                if geo_coords is None and use_country_fallback:
                    ...
                    try:
                        self.logger.debug("Sending request to geopy ...")
                        geo_coords = geolocator.geocode(country)
                    except (geopy.exc.GeocoderTimedOut,
                            geopy.exc.GeocoderServiceError) as e:
                        self.logger.exception(e)
                        self.logger.critical(
                    ...
                    if geo_coords is None:
                        ...
                        self.logger.debug(
                            ...
                            "[{}]".format(geo_coords.address, geo_coords.latitude,
                                          geo_coords.longitude, geo_coords.point))
                self.logger.debug(
                    "Waiting {} second{} for the next geopy request "
                    "...".format(wait_time, add_plural(wait_time)))
                time.sleep(wait_time)
                new_geo_coords = True
                # Update the dict `locations_geo_coords` with the geo coords
                self.logger.debug(
                    "Dictionary `locations_geo_coords` updated!")
                self.locations_geo_coords[location] = geo_coords
            self.logger.debug(
                "Location '{}' [{}] added!".format(location, geo_coords.point))
            cur_locs_geo_coords.setdefault(location, {})
            cur_locs_geo_coords[location].setdefault('geo_coords', geo_coords)
            cur_locs_geo_coords[location].setdefault('count', 0)
            cur_locs_geo_coords[location]['count'] += 1
            counts += 1
        # Sanity check
        n_locs = len(skipped_locs['already_added']) + len(cur_locs_geo_coords)
        ...
        self.logger.info(
            "# of successfully added locations: {}".format(
                len(cur_locs_geo_coords)))
        self.logger.info(
            "# of empty locations: {}".format(
                skipped_locs['empty_locations']))
        self.logger.info(
            "# of duplicated locations: {}".format(
        ...
        if new_geo_coords:
            ...
            self.logger.debug("Saving path: {}".format(
                self.main_config['data_filepaths']['cache_loc_geo_coords']))
            dump_pickle(
                os.path.expanduser(
                    self.main_config['data_filepaths']['cache_loc_geo_coords']),
                self.locations_geo_coords)
            self.logger.debug("Saved!")
        return cur_locs_geo_coords

    @staticmethod
    def _get_european_countries():
        european_countries = set()
        for country_name, values in map_countries().items():
            try:
                continent = country_alpha2_to_continent_code(values['alpha_2'])
                if continent == "EU":
                    european_countries.add(values['alpha_2'])
            except KeyError:
                # Possible cause: Invalid alpha_2, e.g. could be Antarctica
                continue
